...
// $queryrl = "SELECT * FROM tbl_cinformados WHERE id=$id";
// $resultrl = mysqli_query($mysqli, $queryrl);
// $rowrl = mysqli_fetch_array($resultrl);
$CInformado = new ConsInfControlador();
$tCI = $CInformado->tablaConsInfIdControlador($id);

foreach ($tCI as $rowrl) {
  $fechar = $rowrl['fecha'];
  $hora = $rowrl['hora'];
  ...
  $edad = $rowrl['edad'];
  $fechan = $rowrl['fechan'];
  $consentimiento = $rowrl['consentimiento'];
...
$pdf->Cell(0, 7, utf8_decode('CONSENTIMIENTO INFORMADO'), 0, 1, 'C');
...
$pdf->Ln();
$pdf->Cell(0, 7, utf8_decode('CONSENTIMIENTO INFORMADO:'), 0, 1, 'C');
$pdf->MultiCell(0, 7, utf8_decode($GLOBALS['consentimiento']), 0, 1);
