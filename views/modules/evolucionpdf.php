<?php
 ...
 // $id = "4";
  ...
  $tablaEM = $EM->TablaEvolucionMedicaIdControlador($id);
  //mira la tabla de resolucion para la caratula
  // $queryrl = "SELECT * FROM tbl_evolucion WHERE id=$id";
  // $resultrl = mysqli_query($mysqli, $queryrl);
  // $rowrl = mysqli_fetch_array($resultrl);
  foreach($tablaEM as $rowrl){
    $fechar = $rowrl['fecha'];
    $hora = $rowrl['hora'];
    ...
    $edad = $rowrl['edad'];
    $fechan =$rowrl['fechan'];
    $evolucion = $rowrl['evolucion'];
    $mconsulta = $rowrl['mconsulta'];
    $diagprin = $rowrl['diagprin'];
    $diagrel1 = $rowrl['diagrel1'];
    $diagrel2 = $rowrl['diagrel2'];
    $tipodiagprin = $rowrl['tipodiagprin'];
    $alerta = $rowrl['alerta'];
    $consulta = $rowrl['consulta'];
    $complicacion = $rowrl['complicacion'];
    $procedimiento = $rowrl['procedimiento'];
  ...
  $pdf->Cell(0, 7, utf8_decode('EVOLUCION'), 0, 1,'C');
  ...
  $pdf->Cell(0, 7, utf8_decode('ALERTA : '.$GLOBALS['alerta']), 0, 1);
  $pdf->Cell(0, 7, utf8_decode('TIPO DIAGNOSTICO PRINCIPAL : '.$GLOBALS['tipodiagprin']), 0, 1);
  //$pdf->Ln();
  ...
  $pdf->Cell(0, 7, utf8_decode('PROCEDIMIENTO : '.$GLOBALS['procedimiento']), 0, 1);
  //$pdf->Ln();
  $pdf->Cell(0, 7, utf8_decode('CONSULTA : '.$GLOBALS['consulta']), 0, 1);
  $pdf->Cell(0, 7, utf8_decode('COMPLICACION : '.$GLOBALS['complicacion']), 0, 1);

  $pdf->Ln();
  $pdf->Cell(0, 7, utf8_decode('EVOLUCION:'), 0, 1,'C');
  $pdf->MultiCell(0, 7, utf8_decode($GLOBALS['evolucion']), 0, 1);
