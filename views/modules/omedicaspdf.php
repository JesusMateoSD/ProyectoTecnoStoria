...
// $queryrl = "SELECT * FROM tbl_omedicas WHERE id=$id";
// $resultrl = mysqli_query($mysqli, $queryrl);
// $rowrl = mysqli_fetch_array($resultrl);
$OM = new OrdMedControlador();
$tOM = $OM->tablaOrdMedIdControlador($id);

foreach ($tOM as $rowrl) {
  $fechar = $rowrl['fecha'];
  $hora = $rowrl['hora'];
...
$pdf->Cell(0, 7, utf8_decode('ORDEN MEDICA'), 0, 1, 'C');
...
$pdf->Ln();
$pdf->Cell(0, 7, utf8_decode('ORDEN MEDICA:'), 0, 1, 'C');
$pdf->MultiCell(0, 7, utf8_decode($GLOBALS['ordenmedica']), 0, 1);
